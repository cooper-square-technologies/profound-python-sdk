...
class DocumentListParams(TypedDict, total=False):
    ...
    """ID of the organization whose documents to list.

    Required. The caller must be a member of this organization.
    """
    ...
    q: Optional[str]
    """Filter to documents whose name contains this text, case-insensitively.

    Matches only the document's name, never its content — a query that finds nothing
    does not mean the topic is unwritten, only that no title mentions it. Blank or
    ...
    sort: Optional[str]
    """
    Documents are always ordered newest-modified-first (`updated_at DESC`, then
