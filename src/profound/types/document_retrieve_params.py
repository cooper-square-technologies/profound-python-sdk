...
class DocumentRetrieveParams(TypedDict, total=False):
    ...
    named explicitly. The caller must be a member of this organization.
    """

    include_comments: bool
    """Include the document's review comments, each mapped to `{content, context}`.

    On by default — a comment is part of the document's review state, not an aside.
    ...
    include_tabs: bool
    """Include the document's other tabs.

    On by default — a tab body is part of the document, not an aside. Off,
    `additional_tabs` is omitted from the response rather than returned as an empty
    list.
    """

    preview: bool
    """
    Bound every body in the response — `content_markdown` and each tab's — and set
