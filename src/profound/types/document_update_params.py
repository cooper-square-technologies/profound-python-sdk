...
class DocumentUpdateParams(TypedDict, total=False):
    ...
    Renaming sets a permanent lock: once a document is renamed through this route,
    its title stops following the first heading of its content, for the rest of the
    document's life, and no route can undo the lock. Omit to leave the title as it
    is.
    """

    visibility: Optional[Literal["invited_only", "organization"]]
    """
    New sharing scope: `invited_only` for only the people invited to the document,
